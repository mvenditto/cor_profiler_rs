...
class SigParserWrapper : public SigParser {

    private:
        UINT nIndentLevel;

    public:
        SigParserWrapper() { nIndentLevel = 0; }
    ...
    protected:
        ...
        {
            printf("Type generic type variable: number: '%d'\n", number);
        }
        ...
        {
            printf("Type generic member variable: number: '%d'\n", number);
        }

        // the type will be a value type
        virtual void NotifyTypeValueType()
        {
            printf("Type value type\n");
        }
        ...
        {
            printf("Type sz array\n");
        }
};

extern "C" {
    
    typedef struct SigParserCallbacks {
        void (*notify_begin_method)(sig_elem_type elem_type);
    };

    __declspec(dllexport) HRESULT __cdecl parse_signature(
        SigParserCallbacks callbacks, sig_byte* sig, ULONG sig_size);
